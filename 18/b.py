...
def buildTree(expression):
	...
	while i < len(expression):
		...
		if c == "(":
			...
			start = i + 1
			length = findEndOfSubExpression(expression[start:])

			# print("Start: " + str(start) + ", Length: " + str(length) + ", SubExpression: " + expression[start:start + length])

			operationNode = Node(None, c)
			operationNode.childs.append(previousNode)
			multiplicationNode = buildTree(expression[start:start + length])
			operationNode.childs.append(multiplicationNode)
...
def findEndOfSubExpression(subExpression):
	depth = 0
